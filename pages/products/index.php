<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Our Products | Online Software</title>


    <!--Awesome fonts-->
    <link rel="stylesheet" href="https://use.fontawesome.com/releases/v5.15.3/css/all.css" integrity="sha384-SZXxX4whJ79/gErwcOYf+zWLeJdY/qpuqC4cAa9rOGUstPomtqpuNWT9wdPEn2fk" crossorigin="anonymous">
    <title>Online Software</title>
    <link rel="stylesheet" href="../../assets/css/style.css">
    <link rel="stylesheet" href="../../assets/css/main.css">
    <link rel="stylesheet" href="../../assets/css/hamburger.css">

    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/4.5.2/css/bootstrap.min.css">

    <link rel="stylesheet" href="../../assets/css/services.css">
    <link rel="stylesheet" href="../../assets/css/footer.css">

    <link rel="stylesheet" href="./style.css">
    <link rel="stylesheet" href="./style1.css">

    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.1.0/jquery.min.js"></script>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/css/bootstrap.min.css" />
    <script src="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/js/bootstrap.min.js"></script>


</head>

<body>
    <div id="main" class="prel h100">

        <div id="prompt" class="prompt pfx w100 h100 dsp-none">
            <div class="prompt-container pabs">

                <div class="pabs t0 cancel lhinit cp dsp-none">&#x292B;</div>
                <div id="promptMessage" class="prompt-message text-center p12">

                </div>
            </div>
        </div>

        <!-- Navbar -->
        <nav class="pfx fl">
            <div class="toggle pabs" id="toggle">
                <div class="burger hamburger" id="burger">
                    <span class="line"></span>
                    <span class="line"></span>
                    <span class="line"></span>
                </div>
            </div>

            <div class="m-logo lhinit dsp-none">Online Software</div>

            <div class="content prel fl flc flsb h100 w100">
                <div class="logo fl flc"></div>
                <div class="cnt fl flc">
                <a class="navEl fl flc cp home" href="../../index.html" data-pos="1">
                        Home
                    </a>
                    <a class="navEl fl flc cp about" href="../about/index.html" >
                        About
                    </a>
                    <a class="navEl fl flc cp " href="../services/index.html" >
                        Services
                    </a>
                   
                    <a class="navEl fl flc cp projects" href="#"  >
                        Products
                    </a>
                    <a class="navEl fl flc cp contact" href="../../index.html#contact" >
                        Contact Us
                    </a>
                    <a class="navEl fl flc cp contact" id="enquire_btn" href="../enquiry/index.php"  >
                        Enquiry
                    </a>
                </div>
                <div id="india" class="fl flc cp abc">
                <img src="../../assets/images/svg/india.svg" alt="ABC Infomedia">
                </div>
            </div>
        </nav>
        <!-- Navbar -->

        <!-- main -->
        <main>
            <section id="products" class="main prel fl flc fldcl">
                <h1 class="main-heading w100 anim-scroll text-center">Our Products</h1>

                <div class="search_bar fl flc fldrw-rev p12">
                    <input type="search" name="search" id="search" placeholder="Search Products..." autocomplete="off">
                    <i class="fa fa-search"></i>
                </div>


                <!-- filter -->
                <!-- <div class="product-filter w100 fl fl-j-fe">
                    <div class="sort fl">
                        <div class="collection-sort fl fldcl">

                            <label>Filter by:</label>
                            <select>
                                <option value="/">All Products</option>
                                <option value="/">Websites</option>
                                <option value="/">Android Apps</option>
                                <option value="/">iOS Apps</option>
                            </select>
                        </div>
                    </div>
                </div> -->

                <!-- <a href="../../assets/json/products.json"></a> -->
                <section id="productResults" class="products fl flc flww flas" style="min-height: 80vh">
                    <!-- <div class="fl flc fldcl p12">
                        <h2>Item Not Found</h2>
                        <h3>Try Using Appropriate Keywords</h3>
                    </div> -->
                </section>
            </section>
        </main>
        <!-- main -->


        <!-- footer -->
        <footer>

            <div class="column">
                <a class="footer_title">Online Software</a>
                <a>
                    A one-stop solution for all your information technology needs and now, we are trying to expand
                    ourselves and endeavor our presence felt across the whole world.
                </a>
                <a href="" class="icon" title="Facebook" class="fl flc"><i class="fab fa-facebook-f"></i></a>
                <a href="" class="icon" title="Instagram" class="fl flc"><i class="fab fa-instagram"></i></a>
                <a href="" class="icon" title="Twitter" class="fl flc"><i class="fab fa-twitter"></i></a>
            </div>
            <div class="column">
                <a class="footer_title">Other Links</a>
                <!-- <a href="#" onclick="alert('Currently unavailable')">Privacy Policy</a> -->
                <a href="../terms/">Terms & Conditions</a>
              
                <a href="./index.html#contact">Contact Us</a>
            </div>
            <div class="column">
                <a class="footer_title">Short Links</a>
                <a href="./index.html#services">Our Services</a>
                <a href="./index.html#projects">Our Products</a>
                <a href="./index.html#about">About Us</a>
            </div>
            <div class="column">
                <a class="footer_title">Latest Products</a>
                <a href="" title="Lorem ipsum dolor sit amet"><img src="../../assets/images/svg/products/banner/chitFund.svg" width="50px" height="50px"></a>
                <a href="" title="Lorem ipsum dolor sit amet"><img src="../../assets/images/svg/products/banner/chitFund.svg" width="50px" height="50px"></a>
                <a href="" title="Lorem ipsum dolor sit amet"><img src="../../assets/images/svg/products/banner/chitFund.svg" width="50px" height="50px"></a>
                <a href="" title="Lorem ipsum dolor sit amet"><img src="../../assets/images/svg/products/banner/chitFund.svg" width="50px" height="50px"></a>
            </div>
            <div class="column">
                <a class="footer_title">Get In Touch</a>
                <a title="Address"><i class="fas fa-map-marker"></i> No.32, Vinayagar Kovil Street, Karungalpalayam,
                    Erode - 638003, Tamil Nadu, India</a>
                <a href="mailto:support@onlinesoftware.co" title="Email"><i class="fas fa-envelope"></i>
                    support@onlinesoftware.co</a>
                <a href="tel:+919790030919" title="Contact"><i class="fas fa-phone-alt"></i> +91-97900-30919</a>
            </div>

            <div class="sub-footer w100 text-center">
                © CopyRights
                <script>
                    document.write(new Date().getFullYear());
                </script> ONLINE SOFTWARE | All rights reserved

            </div>
        </footer>
        <!-- footer -->

    </div>


    <!-- Bootstrap JS Requirements -->
    <script src="https://code.jquery.com/jquery-3.5.1.slim.min.js " integrity="sha384-DfXdz2htPH0lsSSs5nCTpuj/zy4C+OGpamoFVy38MVBnE+IbbVYUew+OrCXaRkfj " crossorigin="anonymous "></script>
    <script src="https://cdn.jsdelivr.net/npm/popper.js@1.16.1/dist/umd/popper.min.js" integrity="sha384-9/reFTGAW83EW2RDu2S0VKaIzap3H66lZH81PoYlFhbGU+6BZp6G7niu735Sk7lN" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.5.2/js/bootstrap.min.js" integrity="sha384-B4gt1jrGC7Jh4AgTPSdUtOBvfO8shuf57BaghqFfPlYxofvL8/KUEfYiJOMMV+rV" crossorigin="anonymous"></script>


    <script src="../../index.js"></script>
    <script src="../../anim.js"></script>
    <script src="./index.js"></script>

    <script src="../../assets/js/jquery.js"></script>

    <!-- <script src="./search.js"></script> -->
    <script>
        $(document).ready(function() {
            $.getJSON('../../assets/json/products.json', function(data) {

                $.each(data, function(key, value) {
                          
                 
                    var str = value.name;
                   
                    str = str.replace(/\s+/g, '-');
                   

            
                 
                    output =
                        '<a href = "../product-preview/' + str + '.php"' +
                        'class = "cardHolder fl flc fldcl p12 cp" >' +
                        '<div> <img src = "../../assets/images/img/' + value.banner + '1.png"' +
                        'width = "100%"' +
                        'height = "200px" > </div>' +
                        '<div class = "fl w100 fldcl" >' +
                        '<div class = "card-title" >' + value.name + '</div>' +
                        // '<div class = "card-summary" >' + value.summary1 + '</div>' +
                        '</div>' +
                        '</a>';
                    products.append(output);
                });
            });

            search = $('#search');
            products = $('#productResults');


            search.keyup(function() {

                //   $('#state').val('');
                products.html('');
                var searchField = $('#search').val();
                var expression = new RegExp(searchField, "i");

                $.getJSON('../../assets/json/products.json', function(data) {

                    if (search.val() == '') {

                        $.each(data, function(key, value) {

                            var str = value.name;
                            str = str.replace(/\s+/g, '-');

                            output =
                                '<a href = "../product-preview/' + str + '.php"' +
                                'class = "cardHolder fl flc fldcl p12 cp" >' +
                                '<div> <img src = "../../assets/images/img/' + value.banner + '1.png"' +
                                'width = "100%"' +
                                'height = "200px" > </div>' +
                                '<div class = "fl w100 fldcl" >' +
                                '<div class = "card-title" >' + value.name + '</div>' +
                                // '<div class = "card-summary" >' + value.summary1 + '</div>' +
                                '</div>' +
                                '</a>';
                            products.append(output);
                        });

                    }

                    $.each(data, function(key, value) {

                        if (value.name.search(expression) != -1) {
                            // console.log(value.name);

                            var str = value.name;
                            str = str.replace(/\s+/g, '-');


                            output =
                                '<a href = "../product-preview/' + str + '.php"' +
                                'class = "cardHolder fl flc fldcl p12 cp" >' +
                                '<div> <img src = "../../assets/images/img/' + value.banner + '1.png"' +
                                'width = "100%"' +
                                'height = "200px" > </div>' +
                                '<div class = "fl w100 fldcl" >' +
                                '<div class = "card-title" >' + value.name + '</div>' +
                                // '<div class = "card-summary" >' + value.summary1 + '</div>' +
                                '</div>' +
                                '</a>';
                            products.append(output);
                        }
                        // else if (value.name.search(expression) == -1) {
                        //     products.html('');
                        //     output =
                        //         '<div class="fl flc fldcl p12"><h2>Item Not Found</h2><h3>Try Using Appropriate Keywords</h3></div>';
                        //     products.append(output);
                        // }

                    });

                    if (products.children().length === 0) { // Checking if list is empty

                        output =
                            '<img src="../../assets/images/svg/undraw_empty.svg" class="banner">';
                        products.html(output);

                    }
                });
            });
        });
    </script>

</body>

</html>